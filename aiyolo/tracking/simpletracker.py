...
class SimpleTracker:
    ...
    简单的基于距离的跟踪器
    """
    def __init__(self, max_track_length=30, max_missed=80):
        """
        初始化跟踪器
        Args:
        ...
        if not self.tracks:
            ...
            for i, det_center in enumerate(detection_centers):
                ...
                for track_id, track_center in existing_centers.items():
                    ...
                    # 类别不同不匹配
                    if self.tracks[track_id]['class_name'] != detections[i]['class_name']:
                        continue
                ...
                if best_match is not None:
                    ...
                    track_info['current_bbox'] = detections[i]['bbox']
                    track_info['history'].append(detections[i]['bbox'])
                    ...
                    # 预测未来位置
                    future_bbox = self.predict_future_position(detections[i]['bbox'], velocity)
                    ...
                    self.tracks[track_id]['history'].append(detections[i]['bbox'])
